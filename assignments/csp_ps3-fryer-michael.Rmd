...
d$S <- standardize(d$shade)
d$B <- standardize(d$bloom)
```
...
\textit{A DAG of the causal relationship we are exploring:}
```{r, fig.align="center", echo=FALSE}
dag <- dagitty("dag {
  S -> B
  W -> B
}")
coordinates(dag) <- list(x=c(B=0, S=-0.5, W=0.5), y=c(B=.1, S=0, W=0))
drawdag(dag, xlim=c(-1, 1), ylim=c(-1, 1))
...
\textit{Since both our predictor variables and our outcome variable are standardized, we would expect a mean 0 outcome wthen both out predictor variables are 0. I will use a standard deviation prior of 0.5 for water and shade. This is because we expect the individual impacts of water and shade on blooms to be no more highly correlated than 1 meaning one standard deviation change in water or shade implies one standard deviation change in blooms. We will use a standard deviation on our outcome variable of 0.4 to represent our uncertainty but additionaly a looser prior will potentially allow for a better fit on our small sample size.}
...
plot_prior_prediction(
  x=d$W, ylim=c(-2, 2),
  xlab="Water (std)", ylab="Blooms (std)",
  title="Shade Held 0"
)
b <- bS
plot_prior_prediction(
  x=d$S, ylim=c(-2, 2),
  xlab="Shade (std)", ylab="Blooms (std)",
  title="Water Held 0"
)
par(mfrow=c(1, 1))
```

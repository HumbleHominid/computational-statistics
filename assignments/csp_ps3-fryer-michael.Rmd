...
set.seed(rseed)
```
```{r plot_psis_k, echo=FALSE, message=FALSE}
# Plot psis k-values
...
\textit{Given that the model which included the interaction has the highest score in both PSIS and WAIC, it is tempting to assume that}

\textit{As stated above, we observe shifting slopes in \textbf{b)} for} `m3.2` \textit{at varying levels of shade. This would not be able to happen if there was not interaction. Given that} `m3.2` \textit{is also the best performing model, it is tempting to affirm that this implies causation. That is, that water and shade interact to cause blooms. However, we cannot make causal affirmations based on the WAIC and PSIS score. That being said, there is a possible biological explaination for such an interaction to support the causal claim. That being, that tulips need both light and water to produce blooms. This is, of course, obvious. Plants need light and nutrients to grow.}
...
AI was not used for this assignment although, I probably should have because I had a bug in my models where I had `d ~ exp(1)` written instead of `d ~ dexp(1)` so the predictions made no sense and it took me forever to find it.

\newpage
# Appendix

```{r, echo=FALSE}
appendix.labels = c("plot_prior", "plot_post", "calc_interval", "plot_psis_k")
```

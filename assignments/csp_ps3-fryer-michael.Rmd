...
**Collaborators:** \textit{For this assignment, I collaborated with Florian and discussed our independent solutions.}
...
\textit{We observe a KL divergence of 0.022. We expect this value to be small as $p$ and $q$ are relatively "close" together and the KL divergence is the average difference in log probabilities between the two models. This value would be much larger if instead $q=\{0.9, 0.1\}$. Importantly, $D_{KL}(p,q) \neq D_{KL}(q,p)$ in general.}
...
plot_post_prediction_blooms <- function(m, m.name) {
  for (s in -1:1) {
    ...
    for (j in 1:20) lines(-1:1, post[j,], col=col.alpha("black", 0.3))
  }
}
...
```{r, echo=FALSE}
set.seed(rseed)
# NOTE: In the below chunk the messages are suppressed but there are pareto
#   k-values over 0.5 AND 1.0 which points to potential outliers
```
```{r, message=FALSE}
...
par(mfrow=c(1, 1))
```
```{r calc_interval, echo=FALSE}
# Calculate a confidence interval
calc.interval <- function(mu, sigma, z=2.575) { # Default of 99% z-score
  return(mu + c(-1,1) * sigma * z)
}

```
```{r, echo=FALSE}
z <- 1.28 # z-score for 80%
psis.interval <- calc.interval(psis.compare$dPSIS[2], psis.compare$dSE[2], z)
waic.interval <- calc.interval(waic.compare$dWAIC[2], waic.compare$dSE[2], z)

cat("80% Confidence Interval\n")
cat("PSIS: ", psis.interval, "\n")
cat("WAIC: ", waic.interval)
```

\textit{When comparing our two models using PSIS and WAIC we see similar results. With each method,} `m3.2` \textit{is expected to perform better (lower values). When comparing the difference, \textbf{dPSIS} and \textbf{dWAIC}, we observe that it is about the same size as the standard error \textbf{SE} which tells us that these models are potentially not substantially different. When calculating a generous 80\% confidence interval (see above) we observe that the whole 80\% interval is not above 0 meaning that these two models potentially overlap. This gets more egregious with more standard CI such as 90\%, 95\%, and 99\%. With that in mind, I would not conclude that these two models are significantly different.}
\newpage
...
set.seed(rseed)
```
```{r plot_psis_k, echo=FALSE, message=FALSE, fig.width=5, fig.align='center'}
# Plot psis k-values
psis <- PSIS(m3.2, pointwise=TRUE)
waic <- WAIC(m3.2, pointwise=TRUE)
plot(psis$k, waic$penalty,
...
```
```{r, echo=FALSE}
cat(sprintf("Max Pareto k: %0.3f\n", max(psis$k)))
over.0.7 <- sum(psis$k > 0.7)
pct <- (sum(psis$k > 0.7) / length(psis$k)) * 100
cat(sprintf("k > 0.7: %d (%0.1f%%)", over.0.7, pct))
```
\textit{When there are Pareto} $k$ \textit{values above 0.5, the Pareto distribution has infinate variance. Still, we will use a cutoff of 0.7 as theory and simulation suggests values} $k < 0.7$ \textit{are still fine to use. That being said, we observe 3 such values with} $k > 0.7$ \textit{which accounts for 11.1\% of our data. Additionally, one such values is nearly double the theoretical cutoff where the variance becomes infinate (0.5). This is very strong evidence towards the presence of outliers. Possible explainations for such outliers are plants that failed to produce blooms despite getting adequate sun and water.}
...
\textit{It is very likely that light (shade) and water interact to affect tulip blooms. When varying the levels of shade in} `m3.2` \textit{we observe a better fit than in the model we did not,} `m3.1`. \textit{This interaction can be explained biologically by the fact that plants need light and nutrients to grow i.e. plants that have more access to light, need more water to support photosynthesis, and vice versa.}
...
\textit{These two interactions are both a result of a collider, however the statistical relationships are different. In problem 2, the outcome has a positive relationship with both predictor variables. However, in problem 3, the outome has a positive relationship with one predictor varible, and a negative reliationship with another. Additionally, the impact of one predictor influences the affect of another resulting in a multiplicative interaction. Pictured below is a graphical representation of what I believe the causal models to look like.}
\vspace{0.25cm}
```{r, echo=FALSE}
par(mfrow=c(1, 2))
```
```{r dags, echo=FALSE, fig.align='center', fig.show='hold', out.width='.49\\linewidth', out.extra='trim=-0.5cm 0pt -0.5cm 0pt'}
# Plot the supposed DAGs for problems 2 and 3
p2.dag <- dagitty("dag {
  A -> M
  H -> M
}")
coordinates(p2.dag) <- list(x=c(A=-0.5, H=0.5, M=0), y=c(A=0, H=0, M=0.5))
drawdag(p2.dag, xlim=c(-1, 1), ylim=c(-1, 1))
p3.dag <- dagitty("dag {
  S -> B
  W -> B
  S -> U [unobserved]
...
coordinates(p3.dag) <- list(x=c(S=-0.5, W=0.5, U=0, B=0), y=c(S=0, W=0, U=0.25, B=0.5))
drawdag(p3.dag, xlim=c(-1, 1), ylim=c(-1, 1), shapes=c(U="c"))
```
```{r, echo=FALSE}
par(mfrow=c(1,1 ))
```
...
AI was not used for this assignment although, I probably should have because I had a bug in my models where I had `sigma ~ exp(1)` written instead of `sigma ~ dexp(1)` so the predictions made no sense and it took me forever to find it.

\newpage
# Appendix

```{r, echo=FALSE}
appendix.labels = c("plot_prior", "plot_post", "calc_interval", "plot_psis_k", "dags")
```

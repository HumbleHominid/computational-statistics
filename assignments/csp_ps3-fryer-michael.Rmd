...
over.0.5 <- sum(psis$penalty > 0.5)
pct <- (sum(psis$penalty > 0.5) / length(psis$penalty)) * 100
cat(sprintf("Values >0.5: %d (%0.1f%%)", over.0.5, pct))
```
\textit{When there are Pareto} $k$ \textit{values above 0.5, their importance weight can be unreliable. This is an indicator of a potential outlier. The highest Pareto} $k$ \textit{value we observe is 0.994 with 6 total observations (22.2\%) being above 0.5. }

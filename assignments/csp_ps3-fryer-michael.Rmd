...
\textit{Score the models with PSIS.}
```{r, echo=FALSE}
set.seed(42)
```
```{r}
...
```
\textit{Score the models with WAIC.}
```{r, echo=FALSE}
set.seed(42)
```
```{r}
...
\textit{In both cross-validation approximators, a lower score is better. With that metric in mind, both WAIC and PSIS expect} `m6.9` \textit{to be a better prediction model. However, it should be noted that both the standard error} (\textbf{SE}) \textit{and prediction penalty} (\textbf{pPSIS} and \textbf{pWAIC}) \textit{are higher in} `m6.9`.
...
\textit{As WAIC and PSIS are estimates for predictability, one cannot, and should not, use them to inform any decisions about a causal model. That is, WAIC and PSIS scores have no bearing on if the model is the "correct" causal model.}

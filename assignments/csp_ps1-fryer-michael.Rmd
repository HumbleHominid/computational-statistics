...
$$P(Swing\ (red)) = \frac{P(V=b|R=r)P(R=r)}{P(V=b|R=r)P(R=r) + P(V=r|R=b)P(R=b)}$$
```{r}
p_blue <- 0.265
# The parties divide the country equally therefore p_red = p_blue
p_red <- p_blue

p_b_v_r <- 0.1 # Probability red votes blue
p_r_v_b <- 0.2 # Probability blue votes red

p_swing_red <- bayes(p_red, p_b_v_r, (1-p_r_v_b))
p_swing_blue <- bayes(p_blue, p_r_v_b, (1-p_b_v_r))
```
```{r, echo=FALSE}
cat("Prob Red Swing Voter:  ", p_swing_red)
cat("\n")
cat("Prob Blue Swing Voter: ", p_swing_blue)
```

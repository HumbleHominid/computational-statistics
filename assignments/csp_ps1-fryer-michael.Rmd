...
\textit{The probability of an administered BinaxNOW test returning to you a positive result is equal to $P(T=1) = P(T=1|C=1)P(I=1) + P(T=1|C=0)P(C=0)$ where $T$ refers to the test result and $C$ refers to the presence of the antigen.}
```{r}
...
\textit{This is the same as above, however we use different values for our priors.}
```{r}
p_infected_nz <- 0.015
p_not_infected_nz <- 1- p_infected_nz

p_pos <- (
  p_pos_g_infected * p_infected_nz + p_pos_g_not_infected * p_not_infected_nz
)
...
-   Calculate $P(C_1|T_1)$ for BinaxNOW vs the competitor’s generation 2 test.
...
-   Plot $P(C_1|T_1)$ vs base rate for both tests on the same graph.
-   Write 2-3 sentences explaining at which base rates each test is preferable and why the preference changes (or doesn’t change).

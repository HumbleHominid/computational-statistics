...
\textit{The probability of an administered BinaxNOW test returning to you a positive result is equal to $P(T=1) = P(T=1|C=1)P(C=1) + P(T=1|C=0)P(C=0) = sentitivity * prior + (1 - specificity) * (1 - prior)$ where $T$ refers to the test result and $C$ refers to the presence of the antigen.}
```{r}
p_pos <- (
...
p_infected_g_pos_binax_second <- bayes(
  p_infected, p_infected_g_pos_binax, specificity_binax
)
...
\textit{In general, we will apply bayesian updating to determine the likelihood that Smith will swing again. Let $R$ represent party registration, and $V$ represent which party Smith voted for. Then:}

$$P(Swing) = P(V=b|R=r)P(R=r) + P(V=r|R=b)P(R=b)$$
And,

\begin{align}
  P(R=b | V=r) &= \frac{P(V=r|R=b)*P(R=b)}{P(Swing)}\\
  P(R=r | V=b) &= \frac{P(V=b|R=r)*P(R=r)}{P(Swing)}
\end{align}

And finally,

$$P(Swing\ Again) = P(R=b|V=r)P(V=r|R=b) + P(V=r|R=b)P(V=b|R=r)$$
...
p_red_given_swing <- (p_blue_swing * p_blue) / p_swing
p_blue_given_swing <- (p_red_swing * p_red) / p_swing

p_swing_again <- p_red_given_swing * p_blue_swing + p_blue_swing * p_red_swing
```
```{r, echo=FALSE}
cat("Prob Swing Again: ", p_swing_again)
```
...
```{r, include=FALSE}
# Original Code
# The thee parties divide the country equally
p_red <- 1/3
p_blue <- 1/3
p_green <- 1/3

p_red_swing <- 0.1 + 0.05
p_blue_swing <- 0.15 + 0.05
p_green_swing <- 0.1 + 0.2

p_swing <- (
  p_red_swing * p_red +
  p_blue_swing * p_blue +
  p_green_swing * p_green
)

p_red_given_swing <- (p_red_swing * p_red) / p_swing
p_blue_given_swing <- (p_blue_swing * p_blue) / p_swing
p_green_given_swing <- (p_green_swing * p_green) / p_swing

p_swing_again <- p_red_given_swing * p_red_swing +
  p_blue_given_swing * p_blue_swing +
  p_green_given_swing * p_green_swing
```
```{r}
...
ChatGPT was used to implement Bayes' Theorm in question 3 the R way (using operator broadcasting). My previous implementation is available above, but omitted from the report.

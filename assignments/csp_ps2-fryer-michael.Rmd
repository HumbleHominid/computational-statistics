...
drawdag(fox_dag, xlim=c(-1, 1), ylim=c(-1, 1), shapes=c(A="c", F="c"), radius=4)
```
...
plot(NULL, xlim=range(d$area), ylim=c(-1, 12), xlab="Area sq-km", ylab="Weight")
...
area.seq <- seq(from=0, to=5.5, by=0.05)
...
sim.weight <- sim(m, data=list(area=area.seq))
weight.PI <- apply(sim.weight, 2, PI, prob=0.89)

# Plot raw data
plot(weight ~ area, data=d, col=col.alpha(rangi2, 0.7))
# Plot MAP line
...
\textit{We observe a $b$ value of 0.06 on average. This suggests that territory size has a very slightly positive effect on weight.}

...
\textit{Based on above, I am assuming that only the prediction variable, area, should be standardized for this model.}
...
\textit{With a mean of 0.02 and a standard deviation of 0.11 we observe neither a strong nor precise relationship, illustrated in the figure above. This analysis tells us that territory size gives very little information about weight.}
...
\textit{Since both our predictor and prediction variable are standardized, we will simulate priors with mean 0 and a small standard deviation. For our slope pior, b, a value of 1 represents a change of one standard deviation in our predicted variable for 1 standard deviation of change in our predictor variable.}
...
\textit{As with part a, we observe neither a strong nor precise association with a predicted mean of -0.02 and a standard deviation of 0.09. As with above, thi implies food availability gives little information about weight. It is notable that what little impact food availability does have on weight appears to be negative in our dataset.}
...
\textit{In the below analysis, I will standardize both the prediction variables and the predictor variable. This is done so we can accurately compare the models to one another.}
...
#   close to 0
a <- rnorm(N, 0, 0.2)
# Using the same mean and sd for bA as bF.
bA <- rnorm(N, 0, 0.5)
# bF from above
bF <- rnorm(N, 0, 0.5)
```
...
\textit{In the above figure, we can see that by incorporating both predictor variables, the posterior association of both with the outcome has increased. We also note, however, that the uncertainty has increases drastically. Observing results like this is a strong indicator of a potential masked relationship.}
...
coordinates(fox_dag) <- list(x=c(U=0, A=-0.5, F=0.5, W=0), y=c(U=0, A=0, F=0, W=1))
drawdag(fox_dag, xlim=c(-1, 1), ylim=c(-1, 1), shapes=c(A="c", F="c", U="c"))
```

```{r}
xseq <- seq(from=min(d$A)-0.15, to=max(d$A)+0.15, length.out=30)
mu <- link(mAF, data=data.frame(A=xseq, F=0))
mu.mean <- apply(mu, 2, mean)
mu.PI <- apply(mu, 2, PI)
plot(NULL,
...
mu <- link(mAF, data=data.frame(F=xseq, A=0))
mu.mean <- apply(mu, 2, mean)
mu.PI <- apply(mu, 2, PI)
plot(NULL,
